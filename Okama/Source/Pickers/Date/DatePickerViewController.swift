...
extension UIAlertController {
   ...
   func addMonthYearPicker(mode: UIDatePicker.Mode, date: Date?, minimumDate: Date? = nil, maximumDate: Date? = nil, action: MonthYearPickerViewController.Action?) {
      let datePicker = MonthYearPickerViewController(mode: mode, date: date, minimumDate: minimumDate, maximumDate: maximumDate, action: action)
      set(vc: datePicker, height: 217)
   }
}
...
final class MonthYearPickerViewController: UIViewController {
   ...
   fileprivate lazy var datePicker: MonthYearPickerView = { [unowned self] in
      $0.addTarget(self, action: #selector(MonthYearPickerViewController.actionForDatePicker), for: .valueChanged)
      return $0
      }(MonthYearPickerView())
